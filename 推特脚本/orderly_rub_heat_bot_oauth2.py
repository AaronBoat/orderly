#!/usr/bin/env python3
"""
Orderly 推特"蹭热点"自动化脚本 (OAuth 2.0 版本)
...
class OAuth2TokenManager:
    ...
    def load_token(self):
        ...
        if not TOKEN_FILE.exists():
            raise FileNotFoundError(
                f"未找到 {TOKEN_FILE}。请先运行 'python3 get_initial_token.py' 获取初始 token"
            )
        
        try:
            with open(TOKEN_FILE, 'r') as f:
                token = json.load(f)
            logger.info("Token 加载成功")
            return token
        except json.JSONDecodeError as e:
            raise ValueError(f"Token 文件格式错误: {e}")
    ...
    def refresh_token(self):
        ...
        if 'refresh_token' not in self.token:
            raise ValueError("Token 中缺少 refresh_token")
        
        logger.info("正在刷新 access token...")
...
class OrderlyRubHeatBot:
    ...
    def create_client(self):
        ...
        try:
            ...
            client = tweepy.Client(
                bearer_token=BEARER_TOKEN,  # 用于搜索
                access_token=access_token,   # 用于发帖
                wait_on_rate_limit=True
            )
            logger.info("Twitter API 客户端创建成功")
            return client
        except Exception as e:
    ...
    def generate_reply(self, post):
        ...
        variations = [
            ...
            'Excellent insight!', 'Couldn\'t agree more!'
        ]
        ...
        reply_text = reply_text.replace(
            'interesting', 
            random.choice(['fascinating', 'compelling', 'intriguing', 'noteworthy'])
        )
        
        return f"{prefix} {reply_text}"
    
    def post_reply(self, post_id, reply_text, retry=True):
        """发布回复，支持自动 token 刷新重试"""
        try:
            ...
            # 如果是认证错误且允许重试，刷新 token 后重试
            if retry and hasattr(e, 'response') and e.response and e.response.status_code in [401, 403]:
                logger.warning(f"认证失败，尝试刷新 token 后重试...")
                try:
                    self.refresh_client()
                    # 递归调用，但不再重试
                    return self.post_reply(post_id, reply_text, retry=False)
                except Exception as refresh_error:
                    logger.error(f"刷新 token 后重试失败: {refresh_error}")
                    return None
            else:
